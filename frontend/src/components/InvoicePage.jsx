...
//only save invoice entry not adding in orders page !!
...
const InvoicePage = () => {
  ...
  const generatePDF = () => {
    ...
    doc.text(`Invoice #: ${invoiceData.invoiceNumber}`, 105, 32, {
      align: "center",
    });
    ...
    autoTable(doc, {
      ...
      body: products.map((product) => [
        ...
        product.quantity,
        {
          content: `Rs.${product.unitPrice.toFixed(2)}`,
          styles: { halign: "right", fontStyle: "bold" },
        },
        {
          content: `Rs.${product.total.toFixed(2)}`,
          styles: { halign: "right", fontStyle: "bold" },
        },
        {
          content: `Rs.${product.downPayment.toFixed(2)}`,
          styles: { halign: "right", fontStyle: "bold" },
        },
      ]),
      styles: {
    ...
    doc.text(`Discount: Rs. ${discount.toFixed(2)}`, 140, finalY + 8, {
      align: "right",
    });
    doc.text(`Tax: Rs. ${tax.toFixed(2)}`, 140, finalY + 16, {
      align: "right",
    });
    ...
    if (invoiceData.notes) {
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      doc.text(`Notes: ${invoiceData.notes}`, 20, finalY + 40, {
        maxWidth: 170,
      });
    }
        ...
        <header className="invoice-header">
          <div className="invoice-title">
            ...
            <div className="invoice-status">
              <span
                className={`status-badge ${invoiceData.status
                  .toLowerCase()
                  .replace(" ", "-")}`}
              >
                {invoiceData.status}
