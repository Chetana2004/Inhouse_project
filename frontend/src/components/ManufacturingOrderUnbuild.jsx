  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:5000/manufacturing-order/unbuild/${orderId}`,
        { reason }
      );
      ...
      setErrorMessage("Failed to submit unbuild request.");
      console.error(err);
    } finally {
      setIsSubmitting(false);
      setTimeout(() => setSuccessMessage(""), 5000);
    ...
    <>
      ...
      <div className="unbuild-order-container">
        <div className="unbuild-order-card">
          <h1 className="unbuild-order-title">Unbuild Order</h1>
          <p className="unbuild-order-subtitle">
            Submit a request to unbuild an existing order. Please provide the
