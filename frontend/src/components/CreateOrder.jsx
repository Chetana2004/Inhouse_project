...
//     { productId: "P001", name: "", quantity: 1, unitPrice: 0, total: 0 },
//   ]);
...
//     e.preventDefault();
//     if (!orderData.customerName || products.some((p) => !p.name)) {
//       alert(
...
const CreateOrder = ({ onSave }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!orderData.customerName || products.some((p) => !p.name)) {
      alert("Please fill in all required fields (customer and all product names)");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/order/create", newOrder);
