...
const ManufacturingOrderScrap = () => {
  ...
  const navigate = useNavigate(); // 🧭 Setup for redirection
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/manufacturing-order/scrap/${formData.orderId}`,
        {
          reason: formData.scrapReason,
          quantity: formData.quantity,
          notes: formData.notes,
          conveyorType: formData.conveyorType,
      ...
      // Redirect to scrap orders list after 2 seconds
      setTimeout(() => {
        navigate("/scrap-orders");
      }, 2000);
    } catch (err) {
      setErrorMessage("Failed to submit scrap order.");
      console.error(err);
    } finally {
      setIsSubmitting(false);
    }
        ...
        <div className="scrap-order-card">
          ...
          <form onSubmit={handleSubmit} className="scrap-order-form">
            <div className="form-row">
              ...
              <div className="form-group">
                <label htmlFor="conveyorType" className="form-label required">
                  Conveyor Type *
                </label>
                <select
                  id="conveyorType"
            ...
            <div className="form-group">
              <label htmlFor="scrapReason" className="form-label required">
                Scrap Reason *
              </label>
              <select
                id="scrapReason"
